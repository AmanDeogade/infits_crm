...
class DonorExcelUploadScreen extends StatefulWidget {
  const DonorExcelUploadScreen({super.key});

  @override
  State<DonorExcelUploadScreen> createState() => _DonorExcelUploadScreenState();
}

class _DonorExcelUploadScreenState extends State<DonorExcelUploadScreen> {
  ...
  Future<void> pickFileAndParseExcel() async {
    try {
      ...
      if (result != null && result.files.single.bytes != null) {
        ...
        // Validate required columns for donor data
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const Text(
                              'Upload an Excel file (.xlsx) to add multiple donor records at once',
                              style: TextStyle(
                                fontSize: 16,
                                color: Color(0xFF2D3748),
                              ),
                  ...
                  // File Upload Area - Same style as campaign screen
                  Expanded(
                    child: Center(
  ...
  Future<void> _uploadData() async {
    ...
    try {
      ...
      // Process each donor record
      for (int i = 0; i < _parsedData.length; i++) {
        try {
          final donor = _parsedData[i];
          
          // Extract donor data from Excel row - looking for exact column names
          final donorName = donor['Name'] ?? donor['name'] ?? '';
          String donationDate = donor['Date'] ?? donor['date'] ?? '';
            ...
            try {
              ...
              if (parts.length == 3) {
                ...
                if (parts[0].length == 2 && parts[1].length == 2 && parts[2].length == 4) {
                  ...
                  print('Date converted: ${donor['Date']} -> $donationDate');
                }
              }
            } catch (e) {
          ...
          // Extract status and images using exact column names
          final status = donor['Status'] ?? donor['status'] ?? 'Not Verified';
          final images = donor['Images'] ?? donor['images'] ?? 'Not Sent';
          ...
          // Debug logging
          print('Creating donor: Name=$donorName, Date=$donationDate, Status=$status, Images=$images');
      ...
      // Show results
      String message = 'Successfully uploaded $successCount donor records to database.';
      if (errorCount > 0) {
